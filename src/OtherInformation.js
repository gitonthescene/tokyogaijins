...
const Choice = ({ nm, label, items, value, ...props }) => {
  ...
  return (
    <FormControl>
      ...
      <Select id={"choice" + nm} value={value} {...props}>
        {listitems}
      </Select>
    </FormControl>
  );
...
const Entry = ({ label, value, ...props }) => {
  return <TextField type="text" label={label} value={value} {...props} />;
};
...
const OtherInformation = ({ contactinfo, setContactinfo }) => {
  ...
  return (
    ...
    >
      <Entry label="Age" value={contactinfo.age} onChange={onChange("age")} />
      <Entry
